...
class TestProduct(BaseProduct):
    ...
    def new_or_update_product(cls, data: dict, products: List["TestProduct"]):
        matching_products = [
            product for product in products if product.name == data.get("name")
        ]
...
class TestAbstractClass(unittest.TestCase):
    ...
    def test_addition(self):
        first_product = TestProduct(
            name="Продукт А", description="", price=10.0, quantity=10
        )
        second_product = TestProduct(
            name="Продукт Б", description="", price=20.0, quantity=5
        )
        assert first_product + second_product == 200.0

    def test_set_price_valid(self):
        test_product = TestProduct(
            name="Тестовый продукт", description="", price=100.0, quantity=5
        )
